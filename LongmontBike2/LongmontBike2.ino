...
//  Serial.print("ProMini3: startup Ok");
  ...
  pinMode(7,INPUT);
    ...
    strip.setPixelColor(44,color1);            // test fiber on 4
...
/*
  ...
  */
  ...
  if(mode >= 0){
    ...
    if(level==0) {bright_cmd=0; sbright_cmd=0; speriod_cmd=12; furperiod=12;}
    if(level==1) {bright_cmd=0.1; sbright_cmd=0.5; speriod_cmd=9; furperiod=9;}
    if(level==2) {bright_cmd=0.4; sbright_cmd=0.75; speriod_cmd=3; furperiod=3;}
    if(level==3) {bright_cmd=1.0; sbright_cmd=1.0; speriod_cmd=1; furperiod=1;}
...
//    }
  ...
  if(mode==1){
    ...
    // --- Furry String -------------------------    
    strip.setPixelColor(91+furled1,0L);       // clear the 4 sprites from prior pass
    if(furled1<49){                           // color 0L is full off
      strip.setPixelColor(91-furled1,0L); 
    }else{
      strip.setPixelColor(257-furled1,0L); 
    }
    strip.setPixelColor(91+furled2,0L);       
    if(furled2<49){                          
      strip.setPixelColor(91-furled2,0L); 
    }else{
      strip.setPixelColor(257-furled2,0L); 
    }
    strip.setPixelColor(91+furled3,0L);       
    if(furled3<49){                          
      strip.setPixelColor(91-furled3,0L); 
    }else{
      strip.setPixelColor(257-furled3,0L); 
    }
...
/*    
    for(led=1;led<79;led++){                    // side leds
      index2+=increment2;
      if(index2>360) index2-=360;
      color1 = getColor(index2,1,bright2);       
      strip.setPixelColor(91+led,color1);       // left side is continuous
