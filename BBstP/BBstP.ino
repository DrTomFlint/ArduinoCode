/* =============================================================
 *  BB2 snow globe hack for St Pat's Day, only show pure green,
 *  feed accelerometer into beat detector for brightness, add one
 *  revolving sprite 0.2 brighter than rest.  Audio is not used.
 *  Hardware is BB1, promini 3.3, Xbee#4
 *  
 ...
 *  Hack for Green Day, 17 Mar 2019
 *  
 ===============================================================*/
...
float finc = 0.25;      // increment much less than 1.0
float hueBase = 0;
int led = 0;
int index = 0;
int index2 = 0;
...
//  if(value<0)value=-value;    // absolute value
  ...
  // ST PATs put accel into beat detect
  value = 10*(abs(ax)+abs(ay)+abs(az)-1);
...
//  hueBase = getHue(ax,ay,az);
  // For St Pat's, use only variations on Green
  hueBase = getHue(ax,2.0,ay);
...
//  index = hueBase + findex;
  ...
  for(led=0;led<NUMPIXELS;led++){
    ...
    if(led<3){
      ...
      color1 = getColor( index2, 1.0, bright);
      if(led == sprite){
        color1 = getColor( index2, 1.0, bright+0.2);
      }
    }
